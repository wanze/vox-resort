...
const bench: ElevationSpec = {
  terraces: [terrace({ level: 1, fromWater: 20 }), terrace({ level: 2, fromWater: 32 })],
  seed: 1,
};
...
describe('elevationFor', () => {
  ...
  it('refuses a terrace that does not rise or fall at all', () => {
    const flat = {
      terraces: [terrace({ level: 1 }), terrace({ level: 1, fromWater: 32 })],
      seed: 1,
    };
  ...
  it('accepts a terrace that steps back down, so the land can fall away again', () => {
    const knoll = {
      terraces: [terrace({ level: 1 }), terrace({ level: 0, fromWater: 32 })],
      seed: 1,
    };
  ...
  it('refuses terraces listed out of order', () => {
    const backwards = {
      terraces: [terrace({ level: 1, fromWater: 32 }), terrace({ level: 2, fromWater: 20 })],
      seed: 1,
    };
    expect(() => elevationFor(plan({ elevation: backwards }))).toThrow(/not landward/);
  });

  it('refuses two steps that meet', () => {
    const touching = {
      terraces: [terrace({ level: 1, fromWater: 20 }), terrace({ level: 2, fromWater: 20.4 })],
      seed: 1,
    };
  ...
  it('refuses two steps that only meet in one column, once their wobble is rounded', () => {
    ...
    const wobbly = {
      terraces: [
        terrace({ level: 1, fromWater: 20, wave: 3 }),
        terrace({ level: 2, fromWater: 24, wave: 3 }),
      ],
      seed: 8,
    };
  ...
  it('refuses a first step that cuts into the sand', () => {
    ...
    const onSand = { terraces: [terrace({ level: 1, fromWater: 4 })], seed: 1 };
...
describe('stepEdgeZ', () => {
  ...
  it('crosses every column boundary without a step in it', () => {
    const wavy = {
      terraces: [terrace({ level: 1, fromWater: 20, wave: 2 })],
      seed: 3,
    };
...
describe('levelAt', () => {
  ...
  it('never steps more than one level between neighbouring tiles', () => {
    const slope = built({
      ...
      elevation: {
        terraces: [
          terrace({ level: 1, fromWater: 20, wave: 2 }),
          terrace({ level: 2, fromWater: 34, wave: 2 }),
        ],
        seed: 9,
      },
    });
