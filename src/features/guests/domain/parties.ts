 ...
 */
export const PARTY_MIX: readonly PartyShape[] = [
...
function shapeFor(random: () => number): PartyShape {
  let roll = random();
...
export function partiesFor(options: PartyOptions): {
  ...
  while (next < count) {
    ...
    const adults = between(random, shape.adults);
    const children = between(random, shape.children);
    const family = familyName(random);
    ...
    parties.push({ kind: shape.kind, family, members });
    next += size;
  }
