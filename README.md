...
Night used to cost four times what day did. It no longer costs anything extra:
the lamps are baked into an irradiance volume once instead of being evaluated per
fragment, which also lights all 85 of them rather than the nearest 16. Coplanar
faces are merged, instances are bucketed into cullable chunks, and the catalogue
is meshed in a worker. See _Rendering_ and _Lighting_ below for the numbers.
...
`pnpm test`, `pnpm preview` to render model previews (see `voxel-gen/README.md`),
and `pnpm bench` to measure the renderer (see _Measuring_).
...
src/
  ...
  features/
    ...
    lighting/
      ...
      domain/lightAnchors.ts  what a lamp on the plot is
      domain/lightGrid.ts     bakes every lamp into an irradiance volume
      bakedLightVolume.ts     the volume as 3D textures + the TSL that reads it
    voxel-world/
      domain/                 sector/section arithmetic, scratch regions, packed writes
      dveEngine.ts            Divine Voxel Engine adapter
      meshCatalogue.ts        runs the mesher in a worker, with a main-thread fallback
      meshWorker.ts           the worker itself
    rendering/
      domain/vertexBuffer.ts  DVE vertex stream -> flat attributes
      domain/greedyMesh.ts    merges coplanar faces into maximal rectangles
      domain/modelAttributes.ts  mesher output -> attribute arrays (worker-safe)
      domain/spatialChunks.ts buckets instances into cullable chunks
      threeScene.ts           WebGPURenderer, camera, ground, sky, OrbitControls
      voxelMeshBuilder.ts     attribute arrays -> Three.js geometries
      instancedWorld.ts       geometries + placements -> InstancedMeshes per chunk
    hud/
      domain/                 FPS sampling, world -> screen projection, label spread
      Hud.tsx, FpsCounter.tsx, ObjectLabels.tsx
    bench/domain/             benchmark configuration, frame statistics, plot tiling
scripts/bench.ts              drives Chrome over CDP and reports what a frame costs
```
