...
describe('anchorsFor', () => {
  it('has nothing to place for an object that declares no light', () => {
    expect(anchorsFor({ key: 'hedge:1', x: 0, z: 0, y: 0 }, [])).toEqual([]);
  });
  ...
  it("takes the light's height off the ground the object is planted in", () => {
    const [anchor] = anchorsFor({ key: 'torch:1', x: 999, z: 999, y: 0 }, [light({ y: 10 })]);
    expect(anchor?.y).toBe(10);
  });

  it('carries a lamp up onto the terrace its post stands on', () => {
    const [anchor] = anchorsFor({ key: 'street-lamp:9', x: 0, z: 0, y: 24 }, [light({ y: 18 })]);
    expect(anchor?.y).toBe(42);
  });

  it('numbers the lights within the object that owns them', () => {
    const anchors = anchorsFor({ key: 'tennis-court:2', x: 0, z: 0, y: 0 }, [
      light(),
      light({ x: 97 }),
    ]);
  ...
  it('keys two placements of the same model apart', () => {
    const first = anchorsFor({ key: 'street-lamp:1', x: 0, z: 0, y: 0 }, [light()]);
    const second = anchorsFor({ key: 'street-lamp:2', x: 64, z: 0, y: 0 }, [light()]);
